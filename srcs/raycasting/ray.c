...
/*   Updated: 2022/06/28 14:27:11 by emtran           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_pic	*choose_texture_wall(t_texture *texture, t_ray *ray)
{
	if (ray->card == 0)
...
t_ray	*ray_texture(t_data *data, t_player *p1, t_ray *ray)
{
	...
	pic = choose_texture_wall(data->game->texture, ray);
	if (ray->card % 2 == 0)
	...
	ray->text_x = (int)(ray->wall_coord * (double)pic->width);
	if ((ray->card % 2 && ray->dir_x > 0)
		|| (ray->card % 2 == 0 && ray->dir_y < 0))
		ray->text_x = (double)pic->width - ray->text_x - 1;
	ray->text_step = 1.0 * pic->height / ray->wall_height;
